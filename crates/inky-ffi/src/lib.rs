...
/// Caller must free the returned string with inky_free().
#[no_mangle]
pub extern "C" fn inky_transform_inline(input: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(input) };
    let html = c_str.to_str().unwrap_or("");
...
/// Caller must free the returned string with inky_free().
#[no_mangle]
pub extern "C" fn inky_migrate(input: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(input) };
    let html = c_str.to_str().unwrap_or("");
...
/// Caller must free the returned string with inky_free().
#[no_mangle]
pub extern "C" fn inky_migrate_with_details(input: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(input) };
    let html = c_str.to_str().unwrap_or("");
    let result = migrate::migrate(html);
    let changes: Vec<String> = result.changes.iter()
...
/// Caller must free the returned string with inky_free().
#[no_mangle]
pub extern "C" fn inky_validate(input: *const c_char) -> *mut c_char {
    let c_str = unsafe { CStr::from_ptr(input) };
    let html = c_str.to_str().unwrap_or("");
...
pub extern "C" fn inky_version() -> *mut c_char {
    CString::new(env!("CARGO_PKG_VERSION")).unwrap_or_default().into_raw()
}
