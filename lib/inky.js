  ...
  switch (type) {
    case self.components.callout:
      if (component.parent() && self.isTableCell(component.parent()[0].name)) {
        output = format('<table><tbody><tr><td class="callout%s">%s</td></tr></tbody></table>', compAttr.class, inner);
      }
      else {
        output = format('<td class="callout %s">%s</td>', compAttr.class, inner);
      }
      break;
    ...
    case self.components.button:
      ...
      if ('href' in compAttr.attributes) {
        inner = format('<a href="%s">%s</a>', compAttr.attributes.href, inner);
      }
      ...
      output = format('<table class="%s"><tr><td><table><tr><td>%s</td></tr></table></td></tr></table>', classes.join(' '), inner);
      break;
    ...
    case self.components.container:
      output = format('<table class="container%s"><tbody><tr><td>%s</td></tr></tbody></table>', compAttr.class, inner);
      break;
    ...
    case self.components.row:
      output = format('<table class="row%s"><tbody><tr>%s</tr></tbody></table>', compAttr.class, inner);
      break;
    ...
    case self.components.blockGrid:
      output = format('<table class="block-grid up-%s"><tr>%s</tr></table>', component.attr('up'), inner);
      break;

    case self.components.menu:
      output = format('<table class="menu"><tr>%s</tr></table>', inner);
      break;

    case self.components.menuItem:
      output = format('<td><a href="%s">%s</a></td>', compAttr.attributes.href, inner);
      break;

    default:
      // If it's not a custom component, return it as-is
      output = format('<tr><td>%s</td></tr>', $.html(element));
  };
