...
const makeColumn = require('./make-column');
...
module.exports = class Inky {
  ...
  static mergeCheerioOpts(opts) {
    ...
    if (typeof (options.decodeEntities) === 'undefined') {
  ...
  releaseTheKraken(xmlString, cheerioOpts) {
    ...
    const tags = this.componentTags.map(tag => {
      if (tag === 'center') {
  ...
  makeColumn() {
    return makeColumn.apply(this, arguments);
  }
};
