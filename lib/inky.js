...
function Inky(options) {
  ...
  this.postBuild = options.postBuild;
}
 ...
 * @param {string} xmlString - Input HTML as a string
 * @param {object} cheerioOpts - Options to pass into cheerio options
 * @param {function} done - Callback if you want releaseTheKraken to run asynchronously
 * @returns {object} Modified HTML as a string
 */
Inky.prototype.releaseTheKraken = function(xmlString, cheerioOpts, done) {
  ...
  if (this.postBuild) {
    if (done) {
      this.postBuild(string, function(str) {
        done(Inky.reInjectRaws(str, raws));
      });
    } else {
      throw "Cannot use postBuild in synchronous version of releaseTheKraken";
    }
  } else {
    if(done) {
      return done(Inky.reInjectRaws(string, raws))
    } else {
